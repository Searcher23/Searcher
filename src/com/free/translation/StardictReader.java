...
//				}
	...
	public List<String> readDef(String name) throws IOException {
		...
		if (idxx >= 0) {
			...
			for (int idx = Math.max(idxx - 4, 0); idx < Math.min(idxx + 4, llIndex.size()); idx++) {
...
//					System.out.println(en);
					...
					if (dictArr == null && raf.length() < Runtime.getRuntime().maxMemory() * 3 / 10) {
						Log.d("readDef", "start read memory max " + Runtime.getRuntime().freeMemory() + "/" + Runtime.getRuntime().maxMemory());
						...
						lret.add(new String(dictArr, 
											...
											value[1],
											"utf-8"));
						Log.d("readDef", "end read memory max " + Runtime.getRuntime().freeMemory() + "/" + Runtime.getRuntime().maxMemory());
